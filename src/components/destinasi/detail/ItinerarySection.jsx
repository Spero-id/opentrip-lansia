      ...
      <div className="space-y-4 sm:space-y-6 relative before:absolute before:inset-0 before:ml-4 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 before:to-transparent">
        {dest.itinerary?.map((item, idx) => (
          <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
            <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full border-4 border-white bg-[#df7224] text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 font-bold text-[10px] sm:text-xs">
              {item.day}
            </div>
            <div className="w-[calc(100%-2.5rem)] md:w-[calc(50%-2rem)] p-3.5 sm:p-5 rounded-2xl bg-white shadow-sm border border-gray-100 transition-all hover:shadow-md hover:border-[#df7224]/30">
