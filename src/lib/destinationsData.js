export const destinationsData = [
  {
    id: 1,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQw8p4vVW46w8v2EDTYS5ZN08gcBlEyL2Hq2n-oDk588w&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQw8p4vVW46w8v2EDTYS5ZN08gcBlEyL2Hq2n-oDk588w&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Kuta Bali",
    location: "Bali, Indonesia",
    rating: 4.8,
    reviewCount: 3241,
    priceMin: 350000,
    priceMax: 750000,
    category: "Pantai",
    isSeniorFriendly: true,
    description: "Pantai Kuta adalah salah satu tempat wisata paling terkenal di Bali. Terkenal dengan garis pantai berpasir putih yang panjang, ombak yang cocok untuk berselancar pemula, dan pemandangan matahari terbenam (sunset) yang sangat memukau.",
    facilities: ["Akses kursi roda ke area pantai", "Toilet khusus lansia", "Area istirahat berbayang", "Jalur pejalan kaki rata"],
    accessibilityInfo: "Akses kursi roda ke area pantai, toilet khusus lansia, area istirahat berbayang, jalur pejalan kaki rata yang aman untuk lansia.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 2,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ4HXrHCu5wU0hTKdf2vfJj5ZiXuH3LEUeh5s2vEDS6mYWKlFLeAP91yNQ&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ4HXrHCu5wU0hTKdf2vfJj5ZiXuH3LEUeh5s2vEDS6mYWKlFLeAP91yNQ&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Candi Borobudur",
    location: "Jawa Tengah, Indonesia",
    rating: 4.7,
    reviewCount: 5812,
    priceMin: 150000,
    priceMax: 450000,
    category: "Budaya",
    isSeniorFriendly: true,
    description: "Candi Borobudur adalah candi Buddha terbesar di dunia yang dibangun pada abad ke-9. Merupakan salah satu situs warisan dunia UNESCO.",
    facilities: ["Shuttle keliling kompleks", "Kursi roda tersedia", "Area istirahat setiap 100 m", "Pemandu ramah lansia"],
    accessibilityInfo: "Shuttle keliling kompleks, kursi roda tersedia, area istirahat setiap 100 m, dan pemandu ramah lansia.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 3,
    image: "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Raja Ampat",
    location: "Papua Barat, Indonesia",
    rating: 4.9,
    reviewCount: 1987,
    priceMin: 2500000,
    priceMax: 6000000,
    category: "Alam",
    isSeniorFriendly: false,
    description: "Kepulauan Raja Ampat di Papua Barat dikenal sebagai surga bawah laut dunia. Memiliki keanekaragaman hayati laut tertinggi di planet bumi.",
    facilities: ["Kapal dengan akses mudah", "Jaket pelampung gratis", "Pemandu berpengalaman", "Perlengkapan snorkeling lengkap"],
    accessibilityInfo: "Kapal dengan akses mudah untuk lansia, jaket pelampung gratis, pemandu berpengalaman, dan perlengkapan snorkeling lengkap.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 4,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS06Y_q43jk7Wjj7CqIaDWoSfDNLKOIbA6Rlf9UUCqfNg&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS06Y_q43jk7Wjj7CqIaDWoSfDNLKOIbA6Rlf9UUCqfNg&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Gunung Bromo",
    location: "Jawa Timur, Indonesia",
    rating: 4.6,
    reviewCount: 4103,
    priceMin: 250000,
    priceMax: 850000,
    category: "Gunung",
    isSeniorFriendly: false,
    description: "Gunung Bromo adalah salah satu gunung berapi aktif paling ikonik di Indonesia. Terkenal dengan lautan pasirnya yang luas, kawah yang megah.",
    facilities: ["Jeep 4x4 ramah lansia", "Jaket & selimut hangat", "Area istirahat", "Pemandu lokal"],
    accessibilityInfo: "Jeep 4x4 ramah lansia, jaket & selimut hangat, area istirahat, dan pemandu lokal yang membantu sepanjang perjalanan.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 5,
    image: "https://regalsprings.co.id/wp-content/uploads/2023/05/danau-toba-1024x682.jpg",
    images: [
      "https://regalsprings.co.id/wp-content/uploads/2023/05/danau-toba-1024x682.jpg",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Danau Toba",
    location: "Sumatera Utara, Indonesia",
    rating: 4.5,
    reviewCount: 2876,
    priceMin: 300000,
    priceMax: 900000,
    category: "Danau",
    isSeniorFriendly: true,
    description: "Danau Toba adalah danau vulkanik terbesar di dunia yang terbentuk akibat letusan supervolcano purba.",
    facilities: ["Akses mobil ke pinggir danau", "Area duduk tersedia", "Toilet bersih", "Restoran ramah keluarga"],
    accessibilityInfo: "Akses mobil sampai pinggir danau, area duduk tersedia, toilet bersih, dan restoran ramah keluarga.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 6,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQTDyE8zdsszP0ow2_trFHr0KTLgTD70M66eDHdS9b6Rg&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQTDyE8zdsszP0ow2_trFHr0KTLgTD70M66eDHdS9b6Rg&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Komodo Island",
    location: "Nusa Tenggara Timur, Indonesia",
    rating: 4.8,
    reviewCount: 1543,
    priceMin: 1200000,
    priceMax: 3500000,
    category: "Alam",
    isSeniorFriendly: false,
    description: "Pulau Komodo merupakan habitat asli kadal terbesar di dunia, biawak komodo. Taman Nasional ini menawarkan keindahan sabana kering yang dramatis.",
    facilities: ["Trekking jalur landai", "Pemandu berlisensi", "Air minum gratis", "Area istirahat"],
    accessibilityInfo: "Trekking jalur landai, pemandu berlisensi, air minum gratis, dan area istirahat di sepanjang rute.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 7,
    image: "https://indonesiajuara.asia/wp-content/uploads/2023/12/Pink-Beach-di-Indonesia.webp",
    images: [
      "https://indonesiajuara.asia/wp-content/uploads/2023/12/Pink-Beach-di-Indonesia.webp",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Pink Labuan Bajo",
    location: "Nusa Tenggara Timur, Indonesia",
    rating: 4.7,
    reviewCount: 892,
    priceMin: 800000,
    priceMax: 2000000,
    category: "Pantai",
    isSeniorFriendly: true,
    description: "Pantai Pink adalah satu dari sedikit pantai berpasir pink di dunia. Warna pink berasal dari organisme mikroskopis Foraminifera yang bercampur dengan pasir putih.",
    facilities: ["Kapal cepat dengan akses aman", "Perlengkapan snorkeling", "Pemandu berpengalaman", "Area berteduh"],
    accessibilityInfo: "Kapal cepat dengan akses aman, perlengkapan snorkeling, pemandu berpengalaman, dan area berteduh di pantai.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 8,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsXP80SjK6zmY64MbZuZRGG1ysnvLvcC6AfnB5xLJfaA&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsXP80SjK6zmY64MbZuZRGG1ysnvLvcC6AfnB5xLJfaA&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Kawah Ijen",
    location: "Jawa Timur, Indonesia",
    rating: 4.6,
    reviewCount: 3217,
    priceMin: 100000,
    priceMax: 400000,
    category: "Gunung",
    isSeniorFriendly: false,
    description: "Kawah Ijen terkenal di seluruh dunia karena fenomena alam langka 'Blue Fire' atau api biru yang hanya bisa dilihat sebelum fajar.",
    facilities: ["Masker gas & senter", "Sewa porter & tandu", "Pemandu berpengalaman", "Jalur trekking bertanda"],
    accessibilityInfo: "Masker gas & senter, tersedia sewa porter & tandu, pemandu berpengalaman, dan jalur trekking bertanda.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 9,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMOIZq857i_80mhPPx71vprVZvcFoO1bTevA4gbS4s9oIZDaIsTKgdX2k&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMOIZq857i_80mhPPx71vprVZvcFoO1bTevA4gbS4s9oIZDaIsTKgdX2k&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Nusa Penida",
    location: "Bali, Indonesia",
    rating: 4.7,
    reviewCount: 2610,
    priceMin: 450000,
    priceMax: 1100000,
    category: "Pantai",
    isSeniorFriendly: true,
    description: "Nusa Penida adalah pulau kecil di sebelah tenggara Bali yang terkenal dengan tebing-tebing karang raksasa.",
    facilities: ["Mobil wisata ber-AC", "Titik pandang aman", "Pemandu lokal", "Air mineral gratis"],
    accessibilityInfo: "Mobil wisata ber-AC, titik pandang yang aman, pemandu lokal, dan air mineral gratis.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 10,
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRTsbdk525VZcZg1D1-6XkvxRdTuC8Hij7ZHTzQqC7New&s=10",
    images: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRTsbdk525VZcZg1D1-6XkvxRdTuC8Hij7ZHTzQqC7New&s=10",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Gili Trawangan",
    location: "Nusa Tenggara Barat, Indonesia",
    rating: 4.5,
    reviewCount: 4401,
    priceMin: 500000,
    priceMax: 1800000,
    category: "Pulau",
    isSeniorFriendly: true,
    description: "Gili Trawangan adalah pulau terbesar di antara tiga pulau kecil (gili) di barat laut Lombok. Pulau bebas polusi ini melarang kendaraan bermotor.",
    facilities: ["Cidomo & sepeda", "Perlengkapan snorkeling", "Pemandu lokal", "Area istirahat"],
    accessibilityInfo: "Transportasi cidomo & sepeda, perlengkapan snorkeling, pemandu lokal, dan area istirahat tersedia.",
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 11,
    image: "https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Masjid Istiqlal",
    location: "DKI Jakarta, Indonesia",
    rating: 4.9,
    reviewCount: 3890,
    priceMin: 50000,
    priceMax: 150000,
    category: "Religi",
    isSeniorFriendly: true,
    description: "Masjid Istiqlal adalah masjid terbesar di Asia Tenggara dan merupakan simbol kebanggaan umat Islam di Indonesia. Memiliki arsitektur yang megah dan ramah lansia dengan fasilitas lift dan kursi roda.",
    highlights: ["Masjid Terbesar SE-Asia", "Wisata Religi & Sejarah", "Fasilitas Ramah Lansia", "Dekat Gereja Katedral"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Wisata Religi', description: 'Tiba di lokasi, tour interior masjid, dan ibadah bersama.' }
    ],
    meetingPoints: [
      { time: '08:30', location: 'Pintu Gerbang Utama Masjid Istiqlal', description: 'Berkumpul di area parkir utama.' }
    ],
    reviewsList: [
      { author: 'Ahmad Fauzi', rating: 5, date: '10 Juli 2026', comment: 'Sangat megah dan sangat nyaman untuk orang tua kami.' }
    ],
  },
  {
    id: 12,
    image: "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80"
    ],
    title: "Pura Besakih",
    location: "Bali, Indonesia",
    rating: 4.8,
    reviewCount: 2450,
    priceMin: 200000,
    priceMax: 500000,
    category: "Religi",
    isSeniorFriendly: true,
    description: "Pura Besakih adalah pura terbesar dan tersuci di Bali yang terletak di lereng Gunung Agung. Menawarkan keheningan dan keindahan arsitektur Bali purba.",
    highlights: ["Pura Ibukota Bali", "Wisata Religi Bali", "Pemandangan Gunung Agung", "Arsitektur Megah"],
    itinerary: [
      { day: 1, title: 'Wisata Spiritual Pura Besakih', description: 'Menikmati keheningan pura dan sejarah kebudayaan Bali.' }
    ],
    meetingPoints: [
      { time: '09:00', location: 'Area Parkir Pura Besakih', description: 'Bertemu di lobi pusat informasi pengunjung.' }
    ],
    reviewsList: [
      { author: 'Wayan Putra', rating: 5, date: '15 Juni 2026', comment: 'Tempatnya sangat tenang dan cocok untuk refleksi.' }
    ],
  }
];
